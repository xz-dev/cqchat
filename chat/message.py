    ...
    def send_message(self, str_message):
        if not self.__send_message_api:
            if self.__chat_object_type == 'friend':
                self.__send_message_api = '/openqq/send_friend_message'
            elif self.__chat_object_type == 'group':
                self.__send_message_api = '/openqq/send_group_message'
        return self.__send_message(str_message)

    def __send_message(self, str_message, is_https=0, host='127.0.0.1', port='5000'):
